...
WANDB_PROJECT="fwpkm_eval_ppl_stream"
MASTER_PORT=$(shuf -i 12300-65535 -n 1)
deepspeed --include localhost:0 --master_port $MASTER_PORT --module src.eval_ppl_stream \
    --model_type $MODEL_TYPE \
    --tokenizer_dir mistralai/Mistral-7B-v0.1 \
    --pretrained_config_path $MODEL_CFG \
    --max_seq_len 4096 \
    --filepaths \
        data/fineweb/encoded/mistral32k/stream/test10k.8mt \
        data/lc64/encoded/mistral32k/stream/test.8mt \
        data/lambada/encoded/mistral32k/stream/train.8mt \
