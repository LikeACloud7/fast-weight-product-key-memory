...
class LaCTSWIGLULayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            # print(f"q: {q.shape}, k: {k.shape}, v: {v.shape}, attention_mask: {attention_mask.shape}, q_len: {q_len}")
        ...
        if not self.ttt_nope:
            ...
            fast_q = rearrange(fast_q, "b s (n_h d) -> b n_h s d", n_h=self.num_heads)
            fast_k = rearrange(fast_k, "b s (n_h d) -> b n_h s d", n_h=self.num_heads)

            fast_q, fast_k = apply_rotary_pos_emb(
                fast_q,
                fast_k,
                cos,
                sin,
            ...
            fast_q = rearrange(fast_q, "b n_h s d -> b s (n_h d)", n_h=self.num_heads)
            fast_k = rearrange(fast_k, "b n_h s d -> b s (n_h d)", n_h=self.num_heads)
