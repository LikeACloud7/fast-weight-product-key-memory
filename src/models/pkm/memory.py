...
class HashingMemory(nn.Module):
    ...
    def __init__(
        ...
        mem_query_rmsnorm=True,  # Query MLP RMS norm
        # gating
        ...
        assert not (mem_query_batchnorm and mem_query_rmsnorm), "Cannot use both batchnorm and rmsnorm"
        ...
        # initialize keys
        self.keys = nn.Parameter(torch.zeros(2 * self.heads * int(self.size**0.5), self.k_dim // 2))
    ...
    def forward(self, input):
        ...
        bs = torch.tensor(prefix_shape).prod().item() if len(prefix_shape) > 0 else 1
    ...
    def get_indices(self, query, knn):
        ...
        ).view(bs, self.heads, -1)  # (bs, heads, knn ** 2)
        all_indices = (
...
class QueryMLP(nn.Module):
    def __init__(self, input_dim, heads, k_dim, sizes, bias=False, batchnorm=False, rmsnorm=False):
        super().__init__()
        self.input_dim = input_dim
        self.heads = heads
        self.k_dim = k_dim
        ...
        sizes_ = list(sizes)
        sizes_[-1] = sizes_[-1]
        self.query_mlps = QueryMLP.mlp(sizes_, bias=bias)

        # Norm
        if batchnorm:
            self.norm = nn.BatchNorm1d(sizes_[-1], affine=True, eps=1e-6, momentum=0.1)
        elif rmsnorm:
            from transformers.models.qwen3.modeling_qwen3 import Qwen3RMSNorm

            self.norm = Qwen3RMSNorm(sizes_[-1], eps=1e-6)
        else:
            self.norm = None

    @staticmethod
    def mlp(sizes, bias=True):
        """
